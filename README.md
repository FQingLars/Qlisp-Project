...
```bash
# Скачать бинарник (Linux x86_64, релиз v2.7.0)
...
Книга детально документирует архитектуру серии **v2.3.x** (последний полный синк — v2.3.8). Изменения **v2.4–v2.7** — похороны легаси AOT и мира `qvalue` (v2.4.0, «инвариант одной формы»), JIT copy-and-patch с типовыми слотами и специализацией по наблюдаемым типам (v2.5.0–v2.5.5), образы `SAVE-IMAGE`/`LOAD-IMAGE` (v2.6.0), стенсил-эмиттер HLO и отказ от LLVM-пути (v2.6.1–v2.7.0) — задокументированы в [release notes v2.7.0](https://github.com/FQingLars/Qlisp-Project/releases/tag/v2.7.0); главы про JIT и образы обновляются отдельно. Относительно v2.3.0 в книге отражены: фикс времени жизни градиентов и HLO-хендлов (2.3.1), корректность rule engine и завершение S7 (`SECOND-BEST`, 2.3.2), производительность тензоров/автограда до паритета с PyTorch и дисциплина памяти (soak-тесты, `while` без роста RSS, 2.3.3), нейросимвольное RL-ядро susuwatari T1–T4: REINFORCE, макро-слоты, `EVAL-SANDBOXED`, кодер и маршрутизация по слотам (2.3.4), ROUTE-нода в HLO-графе (2.3.5) и `HLO-ROUTE-GRAD!` — обучение роутера без ленты (2.3.6), граф-как-данные + продакшен-пайплайны + умная печать тензоров (2.3.7) и полноценный `qlisp-lsp` (2.3.8).
...
- Графы с `ROUTE` исполняются fallback-программой (LLVM-путь codegen удалён в v2.7.0; диспетчеризация ROUTE стенсилами — в плане).
