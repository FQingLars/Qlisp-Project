...
CUDA-бэкенд (`src/gpu/kernels.cu`, 36 строк) присутствует в исходниках как каркас, но **не зарегистрирован** в `Device::available_devices()` и **не собирается** в релизном бинарнике v2.2.2 (требует CUDA Toolkit и тестирования на GPU). ROCm-бэкенд — в дорожной карте. Книга честно отражает это: на сегодня QLISP — CPU-early.
