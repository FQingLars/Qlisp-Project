...
> ⚠️ **Инвалидация JIT (v2.5.1+).** Любое (пере)определение `defmacro` или `define-compiler-macro` повышает эпоху и **сбрасывает все JIT-страницы** (`jit::invalidate_all`): раскрытие могло быть «запечено» в скомпилированные тела. Страницы восстанавливаются лениво из master-копий (подробности — гл. 20.5); тела с `macrolet` вообще не компилируются.
