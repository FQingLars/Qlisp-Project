...
Это нужно для оптимизации символьных выражений при стенсил-компиляции HLO-кёрнелов (гл. 10).
...
- `NS-IF`: для смешанного батча исполняется ветвь первого образца (маршрут/guilt per-sample верны, но forward-вычисление — только по взятому маршруту). Per-sample-исполнение есть в графовом пути: ROUTE-нода HLO и её fallback-исполнитель (гл. 10.7); диспетчеризация ROUTE стенсилами — в плане (TODO.md).
